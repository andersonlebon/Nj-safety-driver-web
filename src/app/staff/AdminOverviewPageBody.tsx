...
export async function AdminOverviewPageBody() {
  ...
  const insuranceSlices: DonutSlice[] = [
    { label: "Insured", value: vehicles.insured, color: chartColors.brand },
    {
      label: "Not insured",
  ...
  const inspectionSlices: DonutSlice[] = [
    { label: "Inspected", value: vehicles.inspected, color: chartColors.navy },
    {
      label: "Not inspected",
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <KpiCard
          ...
          accent="stone"
          hint={`${vehicles.insured} insured · ${vehicles.inspected} inspected`}
        />
        <KpiCard
          label="Infractions"
        ...
        <Card>
          ...
          <CardBody>
            <BarChartCard
              ...
              series={[
                { key: "paid", label: "Paid", color: statusColor.paid, stackId: "s" },
                { key: "pending", label: "Pending", color: statusColor.pending, stackId: "s" },
                { key: "unpaid", label: "Unpaid", color: statusColor.unpaid, stackId: "s" },
              ]}
              showLegend
              valueFormat="number"
              tickFormat="month"
            ...
            {topAgents.length === 0 ? (
              ...
              <ol className="space-y-2.5">
                {topAgents.map((p, idx) => {
                  ...
                  const displayName =
                    a?.full_name?.trim() || a?.email || `Agent ${p.label.slice(0, 6)}…`;
                  return (
                    <li key={p.label} className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
