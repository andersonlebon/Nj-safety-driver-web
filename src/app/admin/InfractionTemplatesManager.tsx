...
export function InfractionTemplatesManager({
  ...
  const submit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result = await saveInfractionTemplate(data);
      if (!result.ok) {
  ...
  const toggleActive = (template: Template) => {
    ...
    data.set("code", template.code);
    data.set("label", template.label);
    data.set("amount", String(template.amount));
    data.set("points", String(template.points));
    data.set("category", template.category);
    data.set("active", String(!template.active));
    startTransition(async () => {
      const result = await saveInfractionTemplate(data);
      if (!result.ok) {
        setMessage({ type: "error", text: result.error });
